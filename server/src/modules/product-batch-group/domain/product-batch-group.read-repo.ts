...
export class ProductBatchGroupReadRepo extends Repository<ProductBatchGroupReadEntity> {
  ...
  async getLastOrder(statusId: number): Promise<number | null> {
    const lastItems: (ProductBatchReadEntity | ProductBatchGroupReadEntity)[] =
      [];
    const lastUngroupedBatch = await this.manager.findOne(
      ProductBatchReadEntity,
      {
  ...
  async createFromDto(dto: CreateProductBatchGroupDto & { id: number }) {
    const newEntity = new ProductBatchGroupReadEntity();
  ...
  async moveProductBatchGroup(dto: MoveProductBatchGroupDto): Promise<{
    ...
    const status = await this.manager.findOneOrFail(StatusReadEntity, {
      where: { id: statusId },
    });
  ...
  async findById(id: number): Promise<ProductBatchGroupReadEntity> {
...
export const ProductBatchGroupRepositoryProvider = {
  provide: ProductBatchGroupReadRepo,
  inject: [getRepositoryToken(ProductBatchGroupReadEntity)],
  useFactory: (repository: Repository<ProductBatchGroupReadEntity>) => {
    return new ProductBatchGroupReadRepo(
      repository.target,
      repository.manager,
